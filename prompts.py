    ...
    if llm_name == "groq":
        prompt = ChatPromptTemplate.from_messages(
            [("system", system_prompt),
             MessagesPlaceholder(variable_name="messages"),
             (
                 "system", llama3_begin_template + "Summarize and assess the conversation. Given the conversation above, who should act next?" \
                 " Or is the task complete and should we FINISH? Select one of the : {options}" + llama3_end_template,
              ),]).partial(options = str(options), members = ", ".join(members))
...
def get_search_agent_prompt(llm_name):
    if llm_name == "openai":
        SEARCH_AGENT = "Search for job listing based on user-specified parameters. I want you to DISPLAY job title, company URL, location and a summary. \
                        if unsuccessful, retry with any other alternative keywords upto three times and provide the results"
    elif llm_name == "groq":
        SEARCH_AGENT = llama3_begin_template + "You are a Searcher Agent. \
            ...
            If unsuccessful, retry with any other alternative keywords upto three times and provide the results" + llama3_end_template
        
    return SEARCH_AGENT

def get_analyzer_agent_prompt(llm_name):
    if llm_name == "openai":
        ANALYZER_AGENT = "Analyze the content of the user-uploaded document and matching job listing to recommend the best job fit, detailing the reasons behind the best choice."
        ...
        Also, provide detailing on the reasoning behind the choice" \
        + llama3_end_template
